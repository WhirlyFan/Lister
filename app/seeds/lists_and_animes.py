...
# new animes
    ...
    completed.animes.extend([full_metal_alchemist, demon_slayer, one_piece, steins_gate, gintama, hunterxhunter,
                            vinland_saga, code_geass_2, spirited_away, death_note, fate_zero_2, neon_genesis_end])
    on_hold.animes.extend([mob_psycho_2, cowboy_bebop, my_hero_6,
                          detective_conan, attack_on_titan, steins_gate_0, one_punch_man])
    currently_watching.animes.extend(
        [naruto, bleach1000, kaguya_sama, fruit_basket, mob_psycho_1, banana_fish, nichijou])
    favorites.animes.extend([violet_evergarden, shingeki_no_kyojin_final_season,
                            tokyo_revengers, chainsaw_man, code_geass_1, spyxfamily])
    plan_to_watch.animes.extend(
        [kimi_no_wa, made_in_abyss, princess_mononoke, jujutsu_kaisen, cyberpunk_edgerunners])
    completed2.animes.extend([naruto, demon_slayer, full_metal_alchemist,
                             steins_gate, one_piece, hunterxhunter, gintama])
    currently_watching2.animes.extend(
        [full_metal_alchemist, violet_evergarden, shingeki_no_kyojin_final_season])
    on_hold2.animes.extend([kimi_no_wa])
